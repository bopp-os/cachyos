...
rechunk arch='v3' flavor='base':
    @if [ "{{flavor}}" = "base" ]; then \
        ...
        podman run --rm --mount=type=image,source={{registry}}/{{user}}/cachyos-boppos-base:{{arch}},target=/chunkah \
            ...
            quay.io/coreos/chunkah build --compressed --compression-level 2 --label containers.bootc=1 --max-layers 512 --prune /var/cache/ --prune /var/log/ --prune /tmp/ --prune /var/tmp/ | podman load > /tmp/podman_load_output.txt; \
